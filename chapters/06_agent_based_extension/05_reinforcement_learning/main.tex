...
they can be in.
The general form of a policy is given by
equation~\eqref{eq:state_server_dependent_service_rate}.

The following pseudo-code describes the reinforcement learning algorithm.
At each iteration, the agent receives a utility $U$ from the queueing system
and updates its service rate $s$ based on the utility it received.
A policy update is considered a change in the service rate of a server \(k\) at
a state $(u, v)$ that was visited in the last simulation run (i.e. increasing
or decreasing the value of \(\mu_{k, (u,v)}\) for some \(k\) and \((u, v)\)).
...
The total number of iterations is a parameter that needs to be set to a high
...
at each iteration, the simulation is run for a fixed runtime and a fixed number
of trials.
...
policy for the \(2\) servers for the first \(3\) iterations of the
...
\begin{figure}[H]
    ...
    \caption{Example policy for server \(1\) and server \(2\) at iteration \(1\)}
...
\begin{figure}[H]
    ...
    \caption{Example policy for server \(1\) and server \(2\) at iteration \(2\)}
...
\begin{figure}[H]
    ...
    \caption{Example policy for server \(1\) and server \(2\) at iteration \(3\)}
...
Consider a queueing system with the parameters presented in
Table~\ref{tab:reinforcement_learning_parameters}.

\begin{table}[H]
    \centering
    \caption{Parameter values for the reinforcement learning algorithm
    experiments.}
    \begin{tabular}{||c|c|c|c|c|c|c||}
        ...
        \hline
    \end{tabular}
    \label{tab:reinforcement_learning_parameters}
\end{table}
...
\(U_k^{(3)}\) with \(e = 0.1\) and \(100,\!000\) iterations.
...
\begin{figure}[H]
    ...
    \caption{Utilities (left) and mean service rate (right) of servers from the
    reinforcement learning run using utility function \(U_k^{(3)}\) with
    \(e = 0.1\) and \(500,\!000\) iterations}
    \label{fig:RL_utility3_2_e01}
\end{figure}

Running the algorithm again for a longer runtime of \(500,\!000\) iterations
...
of each server is updated at each iteration based on the visited states of the
system.
...
\begin{equation}\label{eq:no_upper_bound_mean_rate_example}
    (0\times1.0) + \left[(4\times10^{15})\times(2.44\times10^{-19})\right]
    \approx 0.001
\end{equation}
...
The total runtime of the reinforcement learning algorithm is 5000 iterations.
...
\(\lambda_2 = 3.5\) for iterations \(2000\) to \(4000\) and then the arrival
...
The first \(2000\) iterations are identical with the first \(2000\) iterations
of the reinforcement learning run of Figure~\ref{fig:RL_utility7_4_e05}.
At iteration \(2000\), when the arrival rates are increased to \(\lambda_1 = 3\)
...
At iteration \(4000\) the arrival rates are decreased back to \(\lambda_1 =
0.5\) and \(\lambda_2 = 1\).
...
rates change.
Having learned their optimal service rates during the first \(2000\) iterations
