...
Discrete event simulation (DES) is currently one of the most popular modelling
techniques.
It is a method for modelling the behaviour of real-world systems in which the
system is made up of discrete events, each of which has a certain
duration~\cite{DESstewart}.
...
There three main approaches to building DES models are the activity scanning
...
Figure~\ref{fig:diagram_of_queueing_system} some slight modifications are
required.
...
overall capacity \(N\)). 
Waiting zone 2 has a deterministic service rate of \(0\), a total of \(M\)
servers and a queueing capacity of \(M\).
Note here that parameters \(N\) and \(M\) are optional and can be also set to
infinity.
The final parameter to be defined is the routing 
