...
column replaced with a vector of ones.
...
The function \textit{numpy.linalg.solve} from the python library \textit{numpy}
was used for this calculation \cite{2020NumPy-Array}.
...
The functions \textit{integrate.odeint} and \textit{integrate.solve\_ivp} from 
the python library \textit{scipy} \cite{2020SciPy-NMeth} are both used to get
the solution. 
The \textit{odeint} function implements the \textit{lsoda} integration method
from the FORTRAN library odepack, while \textit{solve\_ivp} uses the explicit
Runge-Kutta method of order 5(4) (``RK45'') \cite{solve_ivp_rk45_method}.
...
The function \textit{numpy.linalg.lstsq} from the python library \textit{numpy}
was used for this calculation \cite{2020NumPy-Array}.
