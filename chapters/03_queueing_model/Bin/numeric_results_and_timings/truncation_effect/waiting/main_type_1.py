...
for row in all_sim_unbound_waiting_times:
    output_to_file(
        ...
        filename="data/sim_unbound_waiting_times_type_1.csv",
    )

for row in all_sim_bound_waiting_times_low:
    output_to_file(
        str_list=str(row).replace("[", "").replace("]", ""),
        filename="data/sim_bound_waiting_times_low_type_1.csv",
    )

for row in all_sim_bound_waiting_times_mid:
    output_to_file(
        str_list=str(row).replace("[", "").replace("]", ""),
        filename="data/sim_bound_waiting_times_mid_type_1.csv",
    )

for row in all_sim_bound_waiting_times_high:
    output_to_file(
        str_list=str(row).replace("[", "").replace("]", ""),
        filename="data/sim_bound_waiting_times_high_type_1.csv",
    )

for row in all_markov_waiting_times_low:
    output_to_file(
        str_list=str(row).replace("[", "").replace("]", ""),
        filename="data/markov_waiting_times_low_type_1.csv",
    )

for row in all_markov_waiting_times_mid:
    output_to_file(
        str_list=str(row).replace("[", "").replace("]", ""),
        filename="data/markov_waiting_times_mid_type_1.csv",
    )

for row in all_markov_waiting_times_high:
    output_to_file(
        str_list=str(row).replace("[", "").replace("]", ""),
        filename="data/markov_waiting_times_high_type_1.csv",
    )
